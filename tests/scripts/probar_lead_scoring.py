"""Ejecuta todos los payloads de Lead Scoring y guarda evidencia JSON."""
...
URL = "http://localhost:8000/predict/lead-score"
...
def main() -> None:
    payloads = sorted((ROOT / "tests" / "payloads").glob("lead_*.json"))
    resultado = {
        "fecha_utc": datetime.now(timezone.utc).isoformat(),
        "endpoint": URL,
        "casos": [ejecutar(archivo) for archivo in payloads],
    }
    salida = ROOT / "tests" / "resultados" / "resultados_leads.json"
